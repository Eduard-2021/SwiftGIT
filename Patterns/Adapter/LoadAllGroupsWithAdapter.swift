...
class LoadAllGroupsWithAdapter {
    ...
    private func loadAndSaveAllGroupsInRealm(textForSearchInAllGroups : String){
        MainNetworkServiceProxy(mainNetworkService: MainNetworkService()).groupsSearch(textForSearch: textForSearchInAllGroups, numberGroups: 10, completion: { allSearchedGroups in
            guard
